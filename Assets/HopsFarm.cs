...
public class HopsFarm : SegmentBuilding
{
    ...
    private void Update()
    {
        ...
        if (isActive)
        {
            if (tickCtr > interval)
            {
                if (hops < fieldCapacity)
                {
                    hops += 2;
                    hopsVisuals[(int)(hops/2) - 1].SetActive(true);
                    if (hops >= fieldCapacity)
                    {
    ...
    public void setHarvestTask()
    {
        ...
        () => {
            ...
            {
                ResourceManager.instance.createNewWorldResource(parentHex, ResourceManager.ResourceType.HOPS, this.gameObject.transform.position, 0.3f, true);
            }
            hops = 0;
            foreach (GameObject shroom in hopsVisuals)
            {
